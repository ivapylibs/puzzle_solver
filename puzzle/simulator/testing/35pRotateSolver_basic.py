...
theGridSol = Gridded.buildFrom_ImageAndMask(theImageSol, theMaskSol, theParams=ParamGrid(areaThresholdLower=4000, areaThresholdUpper=200000))

# cv2.imshow('debug', theGridSol.toImage(ID_DISPLAY=True))
# cv2.waitKey()

# ==[2.1] Create a new Grid instance from the images
#

_, epBoard = theGridSol.explodedPuzzle(dx=200, dy=200)
...
for i in range(epBoard.size()):
    epBoard.pieces[i].setPlacement(r=np.array([2000, 100]), offset=True)
