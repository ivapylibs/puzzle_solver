#================================ puzzle.piece.sift ================================
##
# @package  puzzle.piece.sift
# @brief    Uses sift features to establish puzzle piece similarity.
...
class Sift(MatchSimilar):
    """!
    @brief  Uses SIFT feature points with descriptors as an object representation.


    It is not perfect for smaller objects with few features, but it does a good
    enough job to work as a first pass.  If better is needed, this serves as a
    baseline from which to understand the properties of the object set.
    """
    ...
    def __init__(self, tau=10, theThreshMatch=0.5):
        ...
        @param[in]  theThreshMatch  The threshold to determine match (0-1).
        """
    ...
    def compare(self, piece_A, piece_B):
        ...
        # UNUSED CODE. EVENTUALLY SHOULD REMOVE. 2026/07/30 - PAV.
        #
...
#================================= Sift_BoW ================================
#
class Sift_BoW(MatchSimilar):
    """!
    @brief  SIFT matching with color-based grouping filter. 

    Given that SIFT lacks color sensitivity, this part adds a BoW histogram
    feature to the process for differentiating object groups.  If two matches
    are not in the same group, then similarity is set to zero.  Otherwise,
    will use SIFT to generate score.

    Our main use is to group objects into smaller sets for matching, where there
    is some acceptable tolerance for in-group mistakes but non for cross-group
    mistakes.

    Note that nothing prevents a group from being 1-1 (only one piece in group)
    as opposed to being many-1.
    """

    #============================== __init__ =============================
    #
    def __init__(self, gNum, tau=10, theThreshMatch=0.5, qNum=20):
        """
        @brief  Constructor for the puzzle piece sift class.

        @param[in]  tau             Threshold param to determine similarity for SIFT feature.
        @param[in]  theThreshMatch  The threshold to determine match (0-1).
        @param[in]  qNum            Quantization number for BoW.
        """

        self.config.theThreshMatch = theThreshMatch
        self.config.tau    = tau
        self.config.nQuant = qNum

        self.nGroups = gNum
        
        super(Sift_BoW, self).__init__(tau)
